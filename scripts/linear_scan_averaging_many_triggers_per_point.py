...
def script_core(
	...
	# This function assumes that the oscilloscope averaging function was
	# activated beforehand. 
	n_triggers = 1 # This is from an old script, don't want to waste time now changing it.
		...
		with open(ofile_path, 'a') as ofile:
			for n_pos,xy_position in enumerate([(x,y) for x,y in zip(x_positions,y_positions)]):
				...
				for n in range(n_triggers):
					...
					# Reset the averaging at each channel ---
					for ch in [1,2,3,4]:
						current_vdiv = osc.get_vdiv(ch=ch)
						osc.set_vdiv(ch=ch, vdiv=10) # This is to reset the acquisition.
						osc.set_vdiv(ch=ch, vdiv=current_vdiv)
					osc.trig_mode = 'norm'
					sleep(sleep_time)
					osc.trig_mode = 'stop'
					raw_data = {}
					for ch in [1,2,3,4]:
						raw_data[f'CH{ch}'] = osc.get_wf(CH=ch)
...
if __name__ == '__main__':
	...
	X_START = 27.338955078124997e-3 -0.15e-3
	X_STOP = X_START + 2*0.15e-3
	Y_FIXED = 30.283330078124997e-3
	STEP_SIZE = 1e-6
